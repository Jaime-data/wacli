...
- `participants list --json` returns `{groupJid, selfJid, selfLid, participants[{userJid, role, updatedAt}]}`. The roster includes this device, so a consumer counting "everyone else" must subtract it — and must check **both** identities: WhatsApp gives the same device two names that share no digits, and a roster is usually written in LIDs, so `selfJid` alone matches nobody there. Both are read from the session store, reported without the device suffix, and are `""` when they cannot be determined (`selfLid` is always `""` against a store written before whatsmeow grew a `lid` column). An empty `participants` array means no roster is known locally, which is not the same as a group with no members.
