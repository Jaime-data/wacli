...
Every payload is a flat JSON object carrying an `EventType` discriminator. The three
shapes are independent: consumers switch on `EventType` and never have to unwrap a
nested envelope.

`EventType: "message"` is the stored live message payload documented above, unchanged
except for the added discriminator:

```json
{"EventType":"message","Chat":"15551234567@s.whatsapp.net","ID":"3EB0…","SenderJID":"15551234567@s.whatsapp.net","Timestamp":"2026-07-25T10:00:00Z","FromMe":false,"Text":"hi","ChatName":"Alice"}
```
...
string on the wire. `MessageIDs` keeps WhatsApp's batching, and in groups `Sender` is the
participant the receipt came from:
