...
// SyncWebhookEventKind is the flat discriminator posted as "EventType" in every
// webhook payload. Kinds other than message are opt-in via --webhook-events, so
// the default payload stream is byte-identical to the pre-discriminator one
// except for the additive EventType field.
